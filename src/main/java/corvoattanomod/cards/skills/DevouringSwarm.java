...
public class DevouringSwarm extends BaseCard {
    public static final String ID = makeID("DevouringSwarm");
    ...
    {
        boolean goreTriggered = false;
        for(int i = 0; i < magicNumber; i++)
        {
            addToBot(new DamageAction(m, new DamageInfo(p, damage, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.SLASH_DIAGONAL));
            if(SpecialBonuses.checkGore() && !goreTriggered)
            {
